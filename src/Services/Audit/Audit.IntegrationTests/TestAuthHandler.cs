...
/// <see cref="ClaimsPrincipal"/> for tests — there is no real Entra tenant to
/// issue tokens against in CI/local runs. Registered as the default
/// authentication scheme by <see cref="AuditApiFactory"/> so every existing
/// test that predates auth keeps working unmodified (fully-privileged by
/// default); dedicated authorization tests narrow the principal via the
/// headers below to exercise 401/403 paths.
/// </summary>
public sealed class TestAuthHandler : AuthenticationHandler<AuthenticationSchemeOptions>
{
    public const string SchemeName = "Test";

    /// <summary>Presence (any value) makes authentication fail, simulating an anonymous caller.</summary>
    public const string AnonymousHeader = "X-Test-Anonymous";

    /// <summary>Comma-separated <see cref="EntraAppRoles"/> values; omit for a fully-privileged principal.</summary>
    ...
    /// <summary>Overrides the fabricated <c>oid</c> claim; defaults to a fixed test user.</summary>
