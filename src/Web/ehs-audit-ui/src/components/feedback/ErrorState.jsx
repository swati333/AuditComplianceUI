...
export function ErrorState({ problem, onRetry, }) {
    return (<Box sx={{ py: 4, px: 2 }}>
      <Alert severity="error" role="alert" action={onRetry && <Button onClick={onRetry}>Retry</Button>}>
        <AlertTitle>{problem.title ?? 'Something went wrong'}</AlertTitle>
        <Typography variant="body2">{getErrorMessage(problem)}</Typography>
        {problem.traceId && (<Typography variant="caption" color="text.secondary" component="div" sx={{ mt: 1 }}>
