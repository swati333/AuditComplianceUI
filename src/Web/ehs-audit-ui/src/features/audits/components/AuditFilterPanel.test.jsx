...
describe('AuditFilterPanel', () => {
    it('reports search text changes', async () => {
        ...
        renderWithProviders(<AuditFilterPanel filters={baseFilters} onChange={onChange} onRefresh={vi.fn()}/>);
    ...
    it('calls onRefresh when the refresh button is clicked', async () => {
        ...
        renderWithProviders(<AuditFilterPanel filters={baseFilters} onChange={vi.fn()} onRefresh={onRefresh}/>);
        await user.click(screen.getByRole('button', { name: /refresh audits/i }));
        expect(onRefresh).toHaveBeenCalledTimes(1);
    });
    it('disables refresh while isRefreshing is true', () => {
        renderWithProviders(<AuditFilterPanel filters={baseFilters} onChange={vi.fn()} onRefresh={vi.fn()} isRefreshing/>);
