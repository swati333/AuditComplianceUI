...
import { AUDIT_STATUSES, type AuditStatus } from '@/types/domain';
  ...
  return (
    <Stack direction="row" spacing={2} sx={{ flexWrap: 'wrap', alignItems: 'flex-start' }}>
      ...
      <TextField
        ...
        onChange={(event) => onChange({ status: event.target.value as AuditStatus | '' })}
        sx={{ minWidth: 180 }}
      >
      ...
      <TextField
        ...
        type="date"
        size="small"
        value={filters.plannedStartDateFrom}
        onChange={(event) => onChange({ plannedStartDateFrom: event.target.value })}
      ...
      <TextField
        ...
        type="date"
        size="small"
        value={filters.plannedStartDateTo}
        onChange={(event) => onChange({ plannedStartDateTo: event.target.value })}
        slotProps={{ inputLabel: { shrink: true } }}
        sx={{ minWidth: 180 }}
      />
