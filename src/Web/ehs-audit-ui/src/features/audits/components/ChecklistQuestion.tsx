...
import type { AnswerState } from '@/features/audits/checklistUtils';
...
export interface ChecklistQuestionProps {
  ...
  error?: { answerState?: string; answerText?: string };
  disabled?: boolean;
}
      ...
      <Stack spacing={1.5}>
        ...
        <FormControl error={Boolean(error?.answerState)} disabled={disabled}>
          ...
          <RadioGroup
            ...
            name={groupName}
            value={answerState ?? ''}
            onChange={(event) => onAnswerStateChange(event.target.value as AnswerState)}
          >
