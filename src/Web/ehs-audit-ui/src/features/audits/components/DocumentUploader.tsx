...
export interface DocumentUploaderProps {
  ...
  /** Omit (or pass undefined) when the owning entity has no upload endpoint yet — the picker renders disabled with `disabledMessage`. */
...
export function DocumentUploader({
  documents,
  onUpload,
  disabledMessage = 'Document upload is not available yet.',
          ...
          {documents.map((doc) => (
            <ListItem key={doc.id} disableGutters>
              ...
              <ListItemText
                ...
                secondary={`${formatSize(doc.sizeBytes)} · ${formatDate(doc.createdDate)}`}
              />
            </ListItem>
          ))}
        </List>
