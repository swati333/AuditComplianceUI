...
import {
  answerStateToIsCompliant,
  findResponse,
  isCompliantToAnswerState,
} from '@/features/audits/checklistUtils';
import { ChecklistQuestion } from '@/features/audits/components/ChecklistQuestion';
import type { ChecklistQuestion as ChecklistQuestionModel } from '@/features/audits/types';
...
export function AuditChecklistPage() {
  ...
  const { data: checklist, isLoading: checklistLoading } = useGetChecklistByIdQuery(
    audit?.checklistId ?? '',
    { skip: !audit?.checklistId },
  );
  ...
  useEffect(() => {
    if (!checklist || !audit) return;
    const answers: ChecklistFormValues['answers'] = {};
  ...
  if (auditLoading) return <LoadingState label="Loading audit…" />;
  if (isError || !audit) return <ErrorState problem={toProblemDetails(error)} onRetry={refetch} />;
    ...
    return (
      <Stack spacing={3}>
        ...
        <EmptyState
          ...
          description="Assign a checklist to this audit before recording responses."
          actionLabel="Assign a checklist"
          onAction={() => {
  ...
  if (checklistLoading || !checklist) return <LoadingState label="Loading checklist…" />;

  const sortedQuestions = [...checklist.questions].sort((a, b) => a.displayOrder - b.displayOrder);

  async function persistAnswers(values: ChecklistFormValues) {
    setSaveError(null);
    setSaveSuccess(null);
...
function QuestionField({
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useFormContext<ChecklistFormValues>();
  ...
  return (
    <ChecklistQuestion
      ...
      onAnswerStateChange={(state) =>
        setValue(`answers.${questionId}.answerState`, state, { shouldDirty: true })
      }
      onAnswerTextChange={(text) =>
        setValue(`answers.${questionId}.answerText`, text, { shouldDirty: true })
      }
      error={{
