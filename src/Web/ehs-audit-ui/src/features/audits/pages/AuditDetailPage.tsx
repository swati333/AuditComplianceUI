...
function FindingsSummary({ auditId }: { auditId: string }) {
  ...
  const bySeverity = (data?.items ?? []).reduce<Record<string, number>>((counts, finding) => {
...
export function AuditDetailPage() {
  ...
  if (isLoading) return <LoadingState label="Loading audit…" />;
      ...
      >
        ...
        <Stack direction="row" spacing={1.5}>
          ...
          <Button
            ...
            onClick={() => navigate(`/audits/${audit.id}/history`)}
          >
            History
          </Button>
      ...
      <Grid container spacing={3}>
        ...
        <Grid size={{ xs: 12, md: 4 }}>
          ...
          <Paper variant="outlined" sx={{ p: 3, mt: 3 }}>
            ...
            <DocumentUploader
              documents={[]}
              disabledMessage="Audits don't support document attachments yet."
            />
          </Paper>
        </Grid>
      </Grid>
