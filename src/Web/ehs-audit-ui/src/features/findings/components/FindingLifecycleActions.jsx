...
import { useCloseFindingMutation, useRequireFindingActionMutation, useResolveFindingMutation, useStartFindingReviewMutation, useVerifyFindingMutation, } from '@/features/findings/api';
 ...
 */
export function FindingLifecycleActions({ finding }) {
    ...
    const actionError = startState.error ??
        requireState.error ??
        resolveState.error ??
        verifyState.error ??
        closeState.error;
    return (<Stack spacing={2}>
      ...
      <RequirePolicy policy="CanManageFindings">
        <Stack direction="row" spacing={1.5} sx={{ flexWrap: 'wrap' }}>
          {finding.status === 'Open' && (<Button variant="contained" onClick={() => startReview(finding.id)} disabled={startState.isLoading}>
              Start review
            </Button>)}
          {finding.status === 'UnderReview' && (<Button variant="contained" onClick={() => requireAction(finding.id)} disabled={requireState.isLoading}>
              Mark action required
            </Button>)}
          {finding.status === 'ActionRequired' && (<Button variant="contained" onClick={() => resolveFinding(finding.id)} disabled={resolveState.isLoading}>
              Resolve
            </Button>)}
          {finding.status === 'Resolved' && (<Button variant="contained" onClick={() => verifyFinding(finding.id)} disabled={verifyState.isLoading}>
              Verify
            </Button>)}
          {finding.status === 'Verified' && (<Button variant="contained" onClick={() => closeFinding(finding.id)} disabled={closeState.isLoading}>
              Close finding
            </Button>)}
        </Stack>
      </RequirePolicy>
