...
function filtersFromParams(searchParams, lockedAuditId) {
    return {
...
export function FindingsListPage() {
    ...
    const { auditId: lockedAuditId } = useParams();
    ...
    const [filters, setFilters] = useState(() => filtersFromParams(searchParams, lockedAuditId));
    ...
    return (<Stack spacing={3}>
      <Stack direction="row" sx={{ justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 2 }}>
        ...
        <RequirePolicy policy="CanManageFindings">
          <Button variant="contained" startIcon={<AddIcon />} onClick={() => navigate(newFindingHref)}>
            New finding
          </Button>
        </RequirePolicy>
      </Stack>
      ...
      <FindingFilterPanel filters={filters} onChange={handleFilterChange} onRefresh={refetch} isRefreshing={isFetching} lockAuditId={Boolean(lockedAuditId)}/>

      {isError ? (<ErrorState problem={toProblemDetails(error)} onRetry={refetch}/>) : (<DataTable caption="Findings" columns={columns} rows={data?.items ?? []} getRowId={(row) => row.id} getRowSx={(row) => (row.severity === 'Critical' ? { bgcolor: 'error.50' } : undefined)} totalCount={data?.totalCount ?? 0} pageNumber={pageNumber} pageSize={pageSize} onPageChange={(page) => updateUrlParams({ page: String(page) })} onPageSizeChange={(size) => updateUrlParams({ pageSize: String(size), page: '1' })} sortBy={sortBy} sortDirection={sortDirection} onSortChange={(column) => updateUrlParams({
                sortBy: column,
                sortDirection: sortBy === column && sortDirection === 'asc' ? 'desc' : 'asc',
            })} onRowClick={(row) => navigate(`/findings/${row.id}`)} isLoading={isFetching} emptyTitle="No findings found" emptyDescription="Try clearing your filters, or record a new finding from an audit."/>)}
    </Stack>);
}
