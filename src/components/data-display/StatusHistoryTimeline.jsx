 ...
 */
export function StatusHistoryTimeline({ entries, renderStatus, }) {
    if (entries.length === 0) {
    ...
    return (<Stack spacing={0}>
      {entries.map((entry, index) => {
            ...
            return (<Stack key={entry.id} direction="row" spacing={2}>
            ...
            <Box sx={{ pb: isLast ? 0 : 2.5 }}>
              <Stack direction="row" spacing={1} sx={{ alignItems: 'center', flexWrap: 'wrap' }}>
                ...
                {entry.fromStatus && (<Typography color="text.secondary" variant="body2">
                    →
                  </Typography>)}
              ...
              {entry.reason && (<Typography variant="body2" sx={{ mt: 0.5 }}>
                  {entry.reason}
                </Typography>)}
            </Box>
          </Stack>);
