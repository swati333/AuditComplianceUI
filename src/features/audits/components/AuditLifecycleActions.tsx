 ...
 */
export function AuditLifecycleActions({ audit }: { audit: AuditDetail }) {
  ...
  const actionError =
    startState.error ?? completeState.error ?? closeState.error ?? cancelState.error;
  ...
  return (
    <Stack spacing={2}>
      ...
      <Stack direction="row" spacing={1.5} sx={{ flexWrap: 'wrap' }}>
        ...
        {!NON_CANCELLABLE_STATUSES.includes(audit.status) && (
          <RequirePolicy policy="CanManageAudits">
      ...
      <ConfirmationDialog
        ...
        message="This will cancel the audit. This action cannot be undone."
        confirmLabel="Cancel audit"
        destructive
        isConfirming={cancelState.isLoading}
