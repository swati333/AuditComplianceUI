...
function auditWithChecklist(checklistId) {
    return {
        id: auditId,
...
function renderChecklistPage(checklistId = sampleChecklist.id) {
    server.use(http.get(`${serviceBaseUrls.audit}/audits/${auditId}`, () => HttpResponse.json(auditWithChecklist(checklistId))));
    return renderWithProviders(<Routes>
      <Route path="/audits/:auditId/checklist" element={<AuditChecklistPage />}/>
    </Routes>, { initialEntries: [`/audits/${auditId}/checklist`] });
}
describe('AuditChecklistPage', () => {
    ...
    it('blocks submit and shows validation errors when mandatory questions are unanswered', async () => {
        ...
        server.use(http.post(`${serviceBaseUrls.audit}/audits/${auditId}/checklist-responses`, () => {
            saveCalled = true;
