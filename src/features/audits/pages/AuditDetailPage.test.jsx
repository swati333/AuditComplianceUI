...
function renderDetailPage(status) {
    server.use(http.get(`${serviceBaseUrls.audit}/audits/${auditId}`, () => HttpResponse.json(baseAudit(status))), http.get(`${serviceBaseUrls.audit}/audits/${auditId}/status-history`, () => HttpResponse.json([])));
    return renderWithProviders(<Routes>
      <Route path="/audits/:auditId" element={<AuditDetailPage />}/>
    </Routes>, { initialEntries: [`/audits/${auditId}`] });
}
describe('AuditDetailPage status transitions', () => {
    ...
    it('offers "Start audit" for a Planned audit and calls the start endpoint', async () => {
        ...
        server.use(http.post(`${serviceBaseUrls.audit}/audits/${auditId}/start`, () => {
            startCalled = true;
    ...
    it('requires confirming before cancelling an audit', async () => {
        ...
        renderDetailPage('Planned');
        server.use(http.post(`${serviceBaseUrls.audit}/audits/${auditId}/cancel`, () => {
            cancelCalled = true;
