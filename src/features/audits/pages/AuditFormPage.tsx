...
import { auditFormSchema, type AuditFormValues } from '@/features/audits/schemas';
import {
...
function TeamSection({ audit }: { audit: AuditDetail }) {
  ...
  return (
    <Paper variant="outlined" sx={{ p: 3 }}>
          ...
          {audit.teamMembers.map((member) => (
            ...
            >
              <ListItemText
                ...
                secondary={<Chip size="small" label={member.role} sx={{ mt: 0.5 }} />}
              />
            </ListItem>
          ))}
        </List>
      ...
      <ConfirmationDialog
        ...
        message="Remove this member from the audit team?"
        confirmLabel="Remove"
        destructive
...
function ChecklistSection({ audit }: { audit: AuditDetail }) {
...
export function AuditFormPage() {
  ...
  const {
    ...
    error,
    refetch,
  } = useGetAuditByIdQuery(auditId!, { skip: !isEdit });
  ...
  if (isEdit && isLoading) return <LoadingState label="Loading audit…" />;
  ...
  return (
    <Stack spacing={3} sx={{ maxWidth: 720 }}>
      ...
      <ConfirmationDialog
        ...
        message="You have unsaved changes to this audit. Leave without saving?"
