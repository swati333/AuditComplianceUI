...
} from '@/features/findings/schemas';
import {
...
function CommentSection({
  ...
  findingId: string;
  comments: FindingDetail['comments'];
}) {
    ...
    <Paper variant="outlined" sx={{ p: 3 }}>
          ...
          {comments.map((comment) => (
            <ListItem key={comment.id} disableGutters alignItems="flex-start">
              <ListItemText
                ...
                secondary={`${comment.authorName} · ${formatDateTime(comment.createdDate)}`}
              />
            </ListItem>
          ))}
        </List>
      ...
      <FormProvider {...form}>
        <Stack
          ...
          sx={{ mt: 2 }}
        >
          <FormTextField<CommentFormValues> name="text" label="Add a comment" size="small" />
...
function AddDocumentDialog({
  ...
  async function onSubmit(values: AddDocumentFormValues) {
    try {
  ...
  return (
    <Dialog open={file !== null} onClose={onClose} fullWidth maxWidth="xs">
        ...
        <Stack component="form" onSubmit={form.handleSubmit(onSubmit)} noValidate>
          <DialogContent>
            <Stack spacing={2}>
              ...
              <FormTextField<AddDocumentFormValues>
                name="blobReference"
                label="Blob reference"
                required
                helperText="No upload endpoint exists yet — enter the storage reference for this file."
              />
            </Stack>
          </DialogContent>
          <DialogActions>
...
export function FindingDetails({ finding }: { finding: FindingDetail }) {
      ...
      >
        ...
        <Stack direction="row" spacing={1.5}>
          <Button
            ...
            onClick={() => navigate(`/findings/${finding.id}/history`)}
          >
            History
          </Button>
          ...
          <Paper variant="outlined" sx={{ p: 3 }}>
            ...
            {requiresAction && (
              <Chip
                ...
                color="warning"
                variant="outlined"
                label={`${finding.severity} findings require a corrective action`}
              />
            )}
          </Paper>
        ...
        <Grid size={{ xs: 12, md: 4 }}>
          <Paper variant="outlined" sx={{ p: 3 }}>
            ...
            <DocumentUploader
              ...
              onUpload={(file) => setPendingUpload(file)}
