...
describe('RootCauseSection', () => {
    ...
    it('surfaces backend validation errors via ProblemDetails', async () => {
        mockUseAuth.mockReturnValue({ roles: ['Finding.Manage'] });
        server.use(http.post(`${serviceBaseUrls.finding}/findings/f1/root-cause-analysis`, () => HttpResponse.json({ title: 'Validation error', detail: 'Text is required.', errorCode: 'VALIDATION_ERROR' }, { status: 400 })));
