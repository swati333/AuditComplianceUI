...
import {
  ...
  type RootCauseAnalysisFormValues,
...
export function RootCauseSection({ finding }: { finding: FindingDetail }) {
  const [recordRootCause, { isLoading, error }] = useRecordRootCauseAnalysisMutation();
  const form = useForm<RootCauseAnalysisFormValues>({
    resolver: zodResolver(rootCauseAnalysisSchema),
    defaultValues: { text: '' },
  });

  async function onSubmit(values: RootCauseAnalysisFormValues) {
    try {
        ...
        <RequirePolicy policy="CanManageFindings">
          <FormProvider {...form}>
            ...
            <Stack component="form" spacing={1.5} onSubmit={form.handleSubmit(onSubmit)} noValidate>
              <FormTextField<RootCauseAnalysisFormValues>
                name="text"
