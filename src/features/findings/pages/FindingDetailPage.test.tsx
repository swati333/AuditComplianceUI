...
function renderDetailPage(overrides: Record<string, unknown> = {}) {
  server.use(
    http.get(`${serviceBaseUrls.finding}/findings/${findingId}`, () =>
      HttpResponse.json(baseFinding(overrides)),
    ),
  );
...
describe('FindingDetailPage', () => {
  ...
  it('flags that High/Critical severity requires a corrective action', async () => {
    ...
    expect(
      await screen.findByText(/Critical findings require a corrective action/i),
    ).toBeInTheDocument();
  });
  ...
  it('offers Start review for an Open finding and calls the endpoint', async () => {
    ...
    server.use(
      http.post(`${serviceBaseUrls.finding}/findings/${findingId}/start-review`, () => {
        startCalled = true;
  ...
  it('posts a new comment', async () => {
    ...
    server.use(
      http.post(
        `${serviceBaseUrls.finding}/findings/${findingId}/comments`,
        async ({ request }) => {
