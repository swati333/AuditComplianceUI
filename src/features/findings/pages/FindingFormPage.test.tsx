...
describe('FindingFormPage (create)', () => {
  it('shows required-field validation errors and does not submit an empty form', async () => {
    ...
    server.use(
      http.post(`${serviceBaseUrls.finding}/findings`, () => {
        createCalled = true;
  ...
  it('disables the submit button while the create request is in flight (duplicate-submit prevention)', async () => {
    ...
    let callCount = 0;
    server.use(
      http.post(`${serviceBaseUrls.finding}/findings`, async () => {
        callCount += 1;
        await delay(200);
    ...
    await user.type(screen.getByLabelText(/audit id/i), '11111111-1111-1111-1111-111111111111');
  ...
  it('locks the audit id field and pre-fills it when created under /audits/:auditId/findings/new', async () => {
    renderWithProviders(
      ...
      { initialEntries: ['/audits/11111111-1111-1111-1111-111111111111/findings/new'] },
    );
