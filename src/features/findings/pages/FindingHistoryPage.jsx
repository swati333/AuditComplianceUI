...
export function FindingHistoryPage() {
    ...
    const { data: history, isLoading: historyLoading, isError, error, refetch, } = useGetFindingStatusHistoryQuery(findingId);
    if (findingLoading || historyLoading)
        return <LoadingState label="Loading history…"/>;
    if (isError)
        return <ErrorState problem={toProblemDetails(error)} onRetry={refetch}/>;
    return (<Stack spacing={3}>
      <Button component={RouterLink} to={`/findings/${findingId}`} startIcon={<ArrowBackOutlinedIcon />} size="small" sx={{ alignSelf: 'flex-start' }}>
        Back to finding
      </Button>
