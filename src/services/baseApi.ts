...
import { serviceBaseUrls, type ServiceName } from '@/config/env';
 ...
 */
const serviceQueries: Record<ServiceName, ReturnType<typeof fetchBaseQuery>> = Object.fromEntries(
  (Object.entries(serviceBaseUrls) as [ServiceName, string][]).map(([service, baseUrl]) => [
    service,
    fetchBaseQuery({
      baseUrl,
...
) as Record<ServiceName, ReturnType<typeof fetchBaseQuery>>;
 ...
 */
const dynamicBaseQuery: BaseQueryFn<ServiceRequest, unknown, FetchBaseQueryError> = async (
  args,
  api,
  extraOptions,
) => {
